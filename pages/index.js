                ...
                <div style={s.productCard}>
                  ...
                  {result.instructions && (
                    <div style={s.instructionsBox}>
                      ...
                      <div style={s.instructionsText} dangerouslySetInnerHTML={{__html: formatInstructions(result.instructions)}} />
                    </div>
                  )}
                </div>
              </div>
...
function formatInstructions(text) {
  return text
    ...
    .replace(/\[([^\]]+)\]\((https?:\/\/[^)]+)\)/g, '<a href="$2" target="_blank" rel="noopener noreferrer" style="color:#4F8EF7;text-decoration:underline;word-break:break-all;">$1</a>')
    .replace(/\n/g, '<br/>')
}
